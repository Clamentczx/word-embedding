        ...
        for epoch in range(self.num_epochs):
            ...
            with torch.no_grad():
                for batch_data, batch_labels in dev_dataloader:
                    ...
                    dev_label += torch.argmax(batch_labels, 1).tolist()
                    dev_pred_label += torch.argmax(predictions, 1).tolist()
                    
                    f1_macro = f1_score(dev_label, dev_pred_label, average='macro')
            ...
            print(f"Epoch {epoch}: Train Loss: {train_loss:.4f}, Dev Loss={dev_loss:.4f}, Dev Acc={dev_correct/dev_count:.4f}, Dev F1_macro={f1_macro:.4f}")
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for batch_data, batch_labels in test_dataloader:
                ...
                test_label += torch.argmax(batch_labels, 1).tolist()
                test_pred_label += torch.argmax(predictions, 1).tolist()
                
                f1_macro = f1_score(test_label, test_pred_label, average='macro')

            test_loss /= len(test_dataloader)
            print(f"Test Loss={test_loss:.4f}, Test Acc={test_correct/test_count:.4f}, Test F1={f1_macro:.4f}")